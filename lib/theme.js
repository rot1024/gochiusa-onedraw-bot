...
function getNextContext(prevctx) {
  ...
  if (config.characterFree.indexOf(moment().tz(config.timezone).day()) >= 0) {
    return {
      mainThemes: prevContext.mainThemes,
      subThemes: prevContext.subThemes,
      themes: prevContext.themes,
      characterFree: true
    };
  }
